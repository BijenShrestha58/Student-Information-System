import { BlogInterface } from "../../../utils/interfaces/addstudent.interface.tsx";
        ...
        <div key={index} className="grid grid-cols-7 mb-12">
          <div className={"col-span-5 mr-5 "}>
            <div className={"flex-row pb-2"}>
              <div>
                <img
                  className="w-5 h-5 inline mr-2 align-middle rounded-full cursor-pointer"
                  src={user}
                />
                <span
                  className="flex-1 text-center text-sm font-medium align-middle leading-4 cursor-pointer"
                  style={{ color: "#242424" }}
                >
                  {blog.user.username}
                </span>
            ...
            <div className={"pt-1 "} style={{ color: "#6B6B6B" }}>
              <h3
                className={
                  "text-base leading-5 overflow-hidden cursor-pointer max-h-10 line-clamp-2"
                }
              >
                {blog.content}
              </h3>
            ...
            <div
              className={"flex text-sm items-center pt-2"}
              style={{ color: "#6B6B6B" }}
            >
              <div className={"flex align-middle"}>
                {format(new Date(blog.createdAt), "MMMM d")}
              </div>
              <div
              ...
              {blog.blogtypes.map((blogType: BlogType) => (
                <div
                  key={blogType._id}
                  className={
                    "cursor-pointer border border-gray-100 rounded-full mr-2 bg-gray-100 py-0.5 px-2"
                  }
                >
                  {blogType.name}
                </div>
